...
- Not tests: LLM-as-judge evals, golden chat transcripts that need a paid key in CI, pixel tests of the zip. A local Evalite loop (`pnpm --filter @repo/web eval`) is allowed beside the web app for a reviewer with a key; it is not the default test job, and a missing key skips. That loop may grade the New England ranking, the compare chip, and both refusals.
...
- Client UI is the locked zip chrome: signed-out Landing, gated chat, one shared sticky full-bleed site header on Landing, login, chat and `/map` (identity left, a destination to `/`; chat, Map, GitHub and the profile control right, with the current surface marked; signed-in Sign out confirms before ending the session; no overflow at phone width), a dense left thread rail for recents beside the transcript column, the public `/map` skyline as the one 3D surface, no Methodology popover as a page. The rail is chrome over `listThreads` (empty Threads included, standing name until the first question): no search, folders, Settled, unread counts, or timestamps. Preserve accessibility attributes, button semantics, and keyboard/screen-reader affordances when editing UI.
