...
function call(result: unknown): ToolCall {
  return { tool: "queryAirports", args: { region: "New England" }, result: result as never, durationMs: 12 };
}
...
test("the score vector expands to percentile, raw value and weight", () => {
  ...
  assert.deepEqual(first?.vector, [
    { key: "congestion", label: "Congestion", percentile: "88 pctl", barPercent: 88, missing: false, raw: "12.4M enplanements/runway", weight: 35 },
    { key: "unmetFlightDemand", label: "Unmet flight demand", percentile: "81 pctl", barPercent: 81, missing: false, raw: "+6.2 pp", weight: 35 },
    { key: "delay", label: "Delay", percentile: "62 pctl", barPercent: 62, missing: false, raw: "14.8 min", weight: 20 },
    { key: "growth", label: "Growth", percentile: "71 pctl", barPercent: 71, missing: false, raw: "+8.4%", weight: 10 },
  ]);
...
test("caveats are this answer's, gathered off the rows it shows", () => {
  ...
  assert.deepEqual(view?.assumptions, ["Weather delays are excluded.", "Delay is missing for HYA."]);
