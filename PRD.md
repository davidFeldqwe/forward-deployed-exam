...
17. As a signed-in analyst, I want a left thread rail listing my threads — standing New thread copy until I send a question, then that first user question — so that I can reopen work without leaving the transcript.
18. As a signed-in analyst, I want New thread at the top of that rail to create a recents row immediately, so that a new question does not append to an old ranking.
...
- Thread rail on chat: New thread at the top creates an empty Thread in recents at once (standing copy until the first user question, then that question; a second New thread on an empty Thread does not stack rows); the open thread marked current; empty recents explains that a question starts one. Dense and near-black, no search, folders, Settled, "show more", or footer tray. On a narrow viewport it is a drawer a header control opens. The comparison window stays in the header, beside the shared actions.
...
A thread belongs to one user. Title is standing New thread copy until the first user question is sent, then that question. Messages persist user text, assistant text, and tool payloads needed to re-render answer objects. No extra `lastResolvedSet` store beyond what the message list already holds.
